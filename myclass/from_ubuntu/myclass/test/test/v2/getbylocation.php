	...
	while($s = $dbconn->fetch_row($locsched))
	{
		...
		if($s[5] == "0")	
			...
			else
			{
					...
					$eventid = $s[5];
					echo $eventid;
					$querytimeslot = "SELECT collegeid, departmentid, event_scope, event_type FROM faculty_events where eventid = ".$eventid;
					$qevent = $dbconn->query($queryevent);
					$result_event = $dbconn->fetch_row($qevent);
					
					$collegeid = $result_event[0];
					$qevent_college = "SELECT college_code FROM colleges where collegeid = ".$collegeid;
					$revent_college = $dbconn->query($qevent_college);
					$event_college = $dbconn->fetch_row($revent_college);
					$college = $event_college[0];
					
					$departmentid = $result_event[1];
					$qevent_department = "SELECT department_desc FROM departments where departmentid = ".$departmentid;
					$revent_department = $dbconn->query($qevent_department);
					$event_department = $dbconn->fetch_row($revent_department);
					$department = $event_department[0];
					
					
					$scope = $result_event[2];
					

					$typeid = $result_event[3];
					$qevent_type = "SELECT type_desc FROM types where typeid = ".$typeid;
					$revent_type = $dbconn->query($qevent_type);
					$event_type = $dbconn->fetch_row($revent_type);
					$type = strtoupper($event_type[0]);
					...
					$data = $type.'<br/>'.$department.'<br/>'.$college.'<br/>'.$timeslot;	
			}
