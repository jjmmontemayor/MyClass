...
function GridSelectAllSubjects($start, $limit) 
{
	...
	$query = "SELECT s.subjectid, s.subject_code, s.subject_desc, s.typeid, s.faculty_credits, s.room_options, s.room_items, c.college_desc, c.collegeid, d.department_desc, d.departmentid
	FROM colleges c, departments d, subjects s WHERE c.collegeid = s.collegeid AND d.departmentid = s.departmentid 
	ORDER BY subjectid ASC LIMIT ".$limit." OFFSET ".$start;
